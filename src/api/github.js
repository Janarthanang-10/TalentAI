// src/api/github.js
// Fetches public data from GitHub (profile or repo link) and formats it
// as resume-like text so the AI screener can analyze it.
//
// No API key required (60 requests/hour limit).
// Optional: add VITE_GITHUB_TOKEN=... to your .env to raise the limit.
...
async function fetchReadme(owner, repo) {
  try {
    ...
    return null; // repo has no README — not a fatal error
  }
}
...
function buildRepoBlock(repo, readme) {
  const parts = [
    ...
    `Stars: ${repo.stargazers_count} | Forks: ${repo.forks_count} | Last updated: ${repo.pushed_at?.slice(0, 10)}`,
...
/** Builds resume-style text from a full GitHub profile. */
async function buildProfileText(owner) {
  ...
  // Fetch READMEs for the top 3 repos only, to stay within rate limits
...
export async function fetchGitHubData(url) {
  ...
  if (!parsed) {
    throw new Error(
      "Please enter a valid GitHub link, e.g. https://github.com/username or https://github.com/user/repo",
    );
  }

  if (!parsed.repo) {
    return buildProfileText(parsed.owner);
  }
